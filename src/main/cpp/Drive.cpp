...
void Robot::R2Jesu_Drive(void)
{
    ...
    inputAngle = atan2(y, x) * 180/M_PI;
    r = sqrt((x*x) + (y*y));
    fieldOrientedAngle = (ahrs->GetYaw()) + inputAngle;
    newX = r * (cos(fieldOrientedAngle * M_PI/180));
    newY = r * (sin(fieldOrientedAngle * M_PI/180));
   ...
   if (m_Drivestick.GetSquareButton())
    {
        pidOutput1 = m_angleController1.Calculate((m_SwerveAnalog1.GetVoltage() * conversion1), 45);
        m_SwerveTurn1.Set(pidOutput1);
        pidOutput2 = m_angleController2.Calculate((m_SwerveAnalog2.GetVoltage() * conversion2), 135);
        m_SwerveTurn2.Set(pidOutput2);
        pidOutput3 = m_angleController3.Calculate((m_SwerveAnalog3.GetVoltage() * conversion3), 45);
        m_SwerveTurn3.Set(pidOutput3);
        pidOutput4 = m_angleController4.Calculate((m_SwerveAnalog4.GetVoltage() * conversion4), 135);
        m_SwerveTurn4.Set(pidOutput4);
    }
    /*
